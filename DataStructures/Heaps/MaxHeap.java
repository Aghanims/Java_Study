...
class MaxHeap 
{
    ...
    private void percolateUp(int nodeIndex) 
    {
       while (nodeIndex > 0) 
       {
          ...
          if (heapArray[nodeIndex] <= heapArray[parentIndex])             
             return; // No violation, so percolate up is done.        
          else 
          {
             // Swap heapArray[nodeIndex] and heapArray[parentIndex]
             System.out.printf("   percolateUp() swap: %d <-> %d\n", heapArray[parentIndex], heapArray[nodeIndex]); //only for demonstration
    ...
    private void percolateDown(int nodeIndex) 
    {
       ...
       while (childIndex < heapSize) 
       {
          ...
          else 
          {
             ...
             System.out.printf("   percolateDown() swap: %d <-> %d\n", heapArray[nodeIndex], heapArray[maxIndex]); //only for demonstration
    ...
    public int remove() 
    {
       ...
       System.out.println("remove():"); //only for demonstration
